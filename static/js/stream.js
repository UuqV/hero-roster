...
(function() {
  ...
  function createStream() {
    function stream() {
      if (arguments.length > 0 && arguments[0] !== HALT)
        updateStream(stream, arguments[0]);
  ...
  function initStream(stream) {
    ...
    (stream.map = stream["fantasy-land/map"] = map),
      (stream["fantasy-land/ap"] = ap),
      (stream["fantasy-land/of"] = createStream);
    (stream.valueOf = valueOf),
      (stream.toJSON = toJSON),
      (stream.toString = valueOf);
  ...
  function combine(fn, streams) {
    if (!streams.every(valid))
      throw new Error(
        "Ensure that each item passed to stream.combine/stream.merge is a stream"
      );
  ...
  function toJSON() {
    return this._state.value != null &&
      typeof this._state.value.toJSON === "function"
      ? this._state.value.toJSON()
      : this._state.value;
  }
